...
##2. Extracts only the measurements on the mean and standard deviation for each measurement.
  Lables<-read.table(header = FALSE,file = "UCI HAR Dataset/features.txt",colClasses = c("NULL","character"))
  Lables_V<-sapply(Lables,as.character)
  names(All)<-Lables_V
  extracted<-All[Lables_V[grep("mean|std",Lables_V)]]

##3. Uses descriptive activity names to name the activities in the data set
  Activities<-read.table(header = FALSE,file = "UCI HAR Dataset/activity_labels.txt",col.names = c("Activity_code","Activity"))
  Y_test<-read.table(header = FALSE,file = "UCI HAR Dataset/test/y_test.txt",col.names="Activity_code")
  Y_train<-read.table(header = FALSE,file = "UCI HAR Dataset/train/y_train.txt",col.names="Activity_code")
